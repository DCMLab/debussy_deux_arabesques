...
.. code:: bash
   ...
   git clone https://github.com/DCMLab/debussy_deux_arabesques.git
...
* `Claude Debussy – Deux Arabesques <https://github.com/DCMLab/debussy_deux_arabesques/archive/refs/heads/main.zip>`__


Data Formats
============

Each piece in this corpus is represented by five files with identical names, each in its own folder. For example, the *Première Arabesque* has the following files:

-  ``MS3/l066-01_arabesques_premiere.mscx``: Uncompressed MuseScore 3.6.2 file including the music and annotation labels.
-  ``notes/l066-01_arabesques_premiere.notes.tsv``: A table of all note heads contained in the score and their relevant features (not each of them represents an onset, some are tied together)
-  ``measures/l066-01_arabesques_premiere.measures.tsv``: A table with relevant information about the measures in the score.
-  ``chords/l066-01_arabesques_premiere.chords.tsv``: A table containing layer-wise unique onset positions with the musical markup (such as dynamics, articulation, lyrics, figured bass, etc.).
-  ``harmonies/l066-01_arabesques_premiere.harmonies.tsv``: A table of the included harmony labels (including cadences and phrases) with their positions in the score.
...
.. code:: python
   ...
   labels = ms3.load_tsv('harmonies/l066-01_arabesques_premiere.tsv')
   notes = ms3.load_tsv('notes/l066-01_arabesques_premiere.tsv')
...
For questions, remarks etc., please `create an issue <https://github.com/DCMLab/debussy_deux_arabesques/issues>`__ and feel free to fork and submit pull requests.
